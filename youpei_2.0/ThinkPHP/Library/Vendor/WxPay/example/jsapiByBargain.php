...
if (!empty($order['coupon_id'])) {
    $coupon = D("Coupon")->find($order['coupon_id']);
    if (time() > $coupon['end_time'] || !in_array($coupon['status'], [0, 3])) {
...
<div class="order-content">
    <ul>
        <li>商品名称：&nbsp;<span><?php echo $product['title'].' '.$classNormal ?></span></li>
        <li>商品数量：&nbsp;<span><?php echo $order['amount'] . ' ' . $product['unit']; ?></span></li>
        <li>商品总价：&nbsp;<span>￥<?php echo $order['total_price']; ?> 元</span></li>
        <li>代金券抵扣：&nbsp;<span>￥<?php echo $order['cash_coupon_price']; ?></span><span> 元</span></li>
        <li>课程券抵扣：&nbsp;<span>￥<?php echo $order['coupon_price']; ?></span><span> 元</span></li>
        <li>砍价抵扣：&nbsp;<span>￥<?php echo $order['bargain_price']; ?></span><span> 元</span></li>
...
<script type="text/javascript">
    ...
    function callpay() {
        ...
        if (need_pay > 0) {
            ...
            $.post("http://" + window.location.host + "/index.php/WxPay/finishByCoupon", {orderId: orderId
            }, function (result) {
                    if (result.status == 1) {
                        alert(result.msg);
