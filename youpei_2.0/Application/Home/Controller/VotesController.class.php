...
class VotesController extends BaseController
{
    ...
    public function ajaxVoteActivityList()
    {
        $list = M('vote')->where(array('check_status' => 2))->order('work_start_time desc')
            ->field('id,work_start_time,vote_end_time,status,title,image')->select();
        $over = [];//已结束活动
        $proceed = [];//进行中活动
        foreach ($list as $k => $v) {
            ...
            if ($v['status'] == 4 || $v['status'] == 5 || $v['status'] == 6 || $v['vote_end_time'] < time()) {
                ...
                //作品数
                $userWorks[$k] = M('contribution_record')->where(array('vote_id' => $v['id'], 'status' => 2))->getField('id');
        ...
        //投票人数
        $voteUser = M('vote_record')->distinct(true)->field('user_id')->where('is_expend=1')->select();
        $voteUserSum = count($voteUser);
        return show(1, '', ['over' => $over, 'overCount' => $overCount, 'proceed' => $proceed,
            'proceedCount' => $proceedCount, 'voteUserSum' => $voteUserSum]);
    }
    ...
    public function ajaxVoteList()
    {
        ...
        $joinUser = M('contribution_record')->where(array('vote_id' => $_GET['vote_id'], 'status' => 2))
            ->order('vote_count desc')->field('id,type,path,username,title,vote_count,age_bracket')->select();
        ...
        //是否已参加活动 1-未参加 2-已参加
        $isJoin = M('contribution_record')->where(['user_id' => $this->user['id']])->getField('id');
        if ($isJoin) {
            $join = 2;
        } else {
            $join = 1;
    ...
    public function ajaxVoteDetail()
    {
        try {
            ...
            $work = M('contribution_record')->where(['id' => $_GET['id'], 'status' => 2])->field('id,path,username,title,vote_id,vote_count,age_bracket,user_id')->find();
            if (!$work) {
            ...
            //更多作品
            $contributions = M('contribution_record')->where(['vote_id' => $vote['id'], 'id' => array('NEQ', $work['id']), 'status' => 2])
                ->field('id,path,username,title,vote_id,vote_count,age_bracket')->limit(0, 2)->select();
            if ($vote['upload_type'] == 1) {
    ...
    public function ajaxUserWorks()
    {
        ...
        field('id,create_time,path,title,vote_count,vote_id,status,age_bracket')->find();
...
//            不为空
    ...
    public function ajaxUserVote()
    {
        ...
        $worksList = M('contribution_record')->where(array('vote_id' => $_GET['vote_id'], 'status' => 2))
            ->field('id,path,title,username,vote_id,vote_count,create_time,age_bracket')->select();
        ...
        return show(1, '', ['workList' => $workList, 'is_null' => $is_null, 'voteStatus' => $voteStatus, 'upload_type' => $upload_type]);
    }
    ...
    public function ajaxUserBillboard()
    {
        ...
        foreach ($list as $key => $val) {
            if($vote['upload_type'] == 1){
                ...
                $list[$key]['image'] = D('User')->getHeadById($val['user_id']);
            }
    ...
    public function uploadVideo()
    {
        ...
        if (!$rs) {
            ...
            //截取视频第一帧做缩略图
            $videoPath = "http://" . $_SERVER['HTTP_HOST'] . "/Upload/" . $rs[$Filedata]['savepath'] . $rs[$Filedata]['savename'];
            $name = explode('.', $rs[$Filedata]['savename']);
            $imageName = dirname(__FILE__) . "../../../../Upload/" . $rs[$Filedata]['savepath'] . $name[0] . '.jpg';
            $fileName = dirname(__FILE__) . "../../../../Public/image/ffmpeg.exe";
            $result = system("$fileName -i  $videoPath  -ss 2 -vframes 1 $imageName");
            $rs[$Filedata]['savepath'] = "Upload/" . $rs[$Filedata]['savepath'];
    ...
    public function createVotePoster()
    {
        ...
        $voteImages = M('contribution_record')->where(['user_id' => $this->user['id'], 'vote_id' => $_GET['vote_id']])->getField('path');
        $voteImages = json_decode($voteImages);
        ...
        //判断商品图片类型
        $voteImagePathInfo = pathinfo($voteImages[0]);
        switch (strtolower($voteImagePathInfo['extension'])) {
            case 'jpg' :
            case 'jpeg' :
                $gdImage = imagecreatefromjpeg('.' . $voteImages[0]);
                break;
            case 'png' :
                $gdImage = imagecreatefrompng('.' . $voteImages[0]);
                break;
            default :
                $voteImage = file_get_contents('.' . $voteImages[0]);
        ...
        imagettftext($background, 20, 0, 30, 1450, imagecolorallocate($background, 0, 0, 0), "Font/msyh.ttc", '请识别右侧二维码投TA一票吧！');
