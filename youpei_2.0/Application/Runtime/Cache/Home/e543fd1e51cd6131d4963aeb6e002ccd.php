...
<!-- <script src="/Public/js/webuploader/webuploader.html5only.min.js"></script> -->
...
function firstFile(key) {
    ...
    // $(".webuploader-element-invisible").eq(key).click()
}
    //    获取url参数
                ...
                if(data.data.type==2){
                   ...
                   $(".videoImg").show()
                    if(data.data.userUpload.upload_status==2){
                        ...
                        var userName=data.data.userUpload.username;
                        var mobile=data.data.userUpload.mobile;
                        var src=data.data.userUpload.video;
                        var srcImg=data.data.userUpload.path;
                        $("#title").val(title)
                        $("#userName").val(userName)
                        $("#mobile").val(mobile)
                        $("#img").css("height","7.23rem")
                        $("#img").html('<div class="video"><video src="'+src+'" id="video" poster="'+srcImg+'" preload controls controlsList="nodownload"/></video></div>')
                        $(".alert").hide()
                        ...
                        uploadFile({
                            ...
                            callback: function (result) {
                                if (result.status == 1) {
                                    ...
                                    $('#upload_image').val('/' + json.file.savepath + json.file.savevideo);
                                    var src=$("#preview").attr("src")
                                    var imgSrc=$("#video1").attr("src")
                                    if(imgSrc.indexOf("mp4")>-1||imgSrc.indexOf("mp3")>-1){
                                      $("#img").css("height","7.23rem")
                                      $("#img").html('<div class="video"><video src="'+imgSrc+'" poster="'+src+'" preload controls controlsList="nodownload"></video</div>')
                                       Media = document.getElementById("video"); 
                                        $("#video").click(function(){
                                          Media.play()
                                        })
    ...
    function sub(){
      ...
      if(upload_status==2){
        if(statuss==2){
          ...
          var confirm=window.confirm("重新投稿将会换掉之前投稿的作品，您确定更改吗？")
          if(confirm==true){
            $(".sub").show()
            $.ajax({   
              type: "post",  
