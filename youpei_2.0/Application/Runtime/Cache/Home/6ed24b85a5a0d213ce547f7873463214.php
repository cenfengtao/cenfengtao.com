...
<div id="bonfire-pageloader">
 ...
 <!-- Created with SVG-edit - http://svg-edit.googlecode.com/ -->
    ...
    <link rel="stylesheet" href="/Public/css/home/collar_red.css">
<section class="red">
    <div class="integral">
        <span>积分余额：<?php echo ($integral['integral']); ?></span>
    </div>
    <div class="sum-title">
        <span> — 可兑换余额 —</span>
    </div>
    <div class="exchange-money">
        <span class="selected">1元</span>
        <span class="">2元</span>
    </div>
    <input value="1" autocomplete="off" disabled="true" type="hidden" name="" id="count">
    <div class="twobtn">
        <a class="qr-submit left3" id="btnzhuce" onclick="submitintegral()">确认兑换</a>
        <div class="clr"></div>
    </div>
    <div class="explain">
        <ul>
...
<script type="text/javascript">
    $(".exchange-money span").click(function () {
        $(this).addClass("selected").siblings().removeClass("selected");
        var str = parseInt($('.selected').text());
        $("#count").val(str);
    });

    function submitintegral() {
        $('#btnzhuce').removeAttr('onclick');
        ...
        if (num >= 1) {
            $.post("<?php echo U('Personal/exchange');?>", {price: num}, function (res) {
                if (res.status == 1) {
                    alert('兑换成功');
