...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/Public/css/homes/voteActivityList.css"/>
    <style>
        .roll{
            background: white;
...
<script src="/Public/js/homes/voteActivityList.js" type="text/javascript" charset="utf-8"></script>
<script type="text/javascript" src="/Public/Home/js/jweixin-1.0.0.js"></script>
<script>
    $.ajax({
        type:"get",
        url: "/index.php/Vote/votePrize",
        aysnc: true,
        dataType: "json",
        success:function(data){
            if(data.status==1){
                 for(var i=0; i<data.data.length; i++){
                     $(".prize").append(
                            '<li class="swiper-slide">'+data.data[i].title+' '+data.data[i].prizeLevel+' '+data.data[i].username+'  获得'+data.data[i].prize+'! </li>'
                        )
                }
