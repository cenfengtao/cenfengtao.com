...
<style>
    ...
    .Group .list-talent-product{
        margin-bottom: 9rem;
    }
    .Group .original{
        text-decoration: line-through;
    }
    .list-talent-product .price:after{
...
<link rel="stylesheet" href="/Public/css/home/boutique.css">
<div id="page_user" title="<?php echo ($title); ?>" class="kap-page kap-page-current"> 
 <div class="kap-top ty-menu-top">
  ...
  <div class="user-area">
     <div class="ty-user-cover img">
           <img src="/Public/images/1497578116.png" width="100%" height="auto">
     </div>
    </div>
     <div  class="Group">
      <ul class="list-talent-product">
              <?php if(is_array($boutiqueList)): $k = 0; $__LIST__ = array_slice($boutiqueList,0,10,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($k % 2 );++$k;?><li class="item on" onclick="getProduct(<?php echo ($val['id']); ?>)">
                       <img class="thumb" src="<?php echo ($val['pic_url']); ?>">
                        <span class="title"><font style="" color="#1d709c">
                            <?php echo ($val['title']); ?>
                        </font></span>
                        <!--<span class="status"></span>-->
                        <span class="original">原价:<?php echo ($val['price']['original_price']); ?>元/<?php echo ($val['unit']); ?></span>
                        <span class="discount">精品价:</span>
                        <span class="price"><?php echo ($val['price']['now_price']); ?><span>元/<?php echo ($val['unit']); ?></span></span>
                        <!--<div class="condition">
                         <label>10人成班 已有<span>30人</span>报名</label><br/>
                         <time>2017年7月31日截止</time>
                        </div>-->
    ...
    <div class="kap-bottom ty-menu-bottom" style="display: none">
        <div id="tools_idx" class="ty-tools ty-tools-idx ty-tools-current">
            <a href="<?php echo U('Index/index',array('current'=>'1'));?>" style="background: url('/Public/images/1495008448.png') no-repeat 0.7rem 1.2rem; background-size:20% 30%" <?php if($current == 1): ?>class="current"<?php endif; ?> data-icon="">首页</a>
            <!--<i class="border"></i>-->
            <a href="<?php echo U('Discovery/index',array('current'=>'2'));?>" style="background: url('/Public/images/1495008486.png') no-repeat 0.7rem 1.2rem; background-size:20% 30%" class="current" data-icon="" onclick="_hmt.push(['_trackEvent', 'footbar', 'click', '发现'])">发现</a>
            <!--<i class="border"></i>-->
            <a href="<?php echo U('Association/index',array('current'=>'3'));?>" style="background: url('/Public/images/1495008503.png') no-repeat 0.7rem 1.2rem; background-size:20% 30%" <?php if($current == 3): ?>class="current"<?php endif; ?>  data-icon="" onclick="_hmt.push(['_trackEvent', 'footbar', 'click', '消息'])">社群<i id="new_msg_alert"></i></a>
            <!--<i class="border"></i>-->
            <a href="<?php echo U('Personal/index',array('current'=>'4'));?>" <?php if($current == 4): ?>class="current"<?php endif; ?> style="background: url('/Public/images/1495008528.png') no-repeat 0.7rem 1.2rem; background-size:15% 30%" onclick="_hmt.push(['_trackEvent', 'footbar', 'click', '我的'])">个人中心</a>
        </div>
    </div>
    ...
    $(function () {
        $(window).scroll(function () {
            ...
            if (scrollTop + windowHeight == scrollHeight) {
                ...
                var url = "<?php echo U('Boutique/loadingProduct');?>";
                var params = {};
                params.npage = npage;
                $.post(url, params, function (result) {
                    if (result.status == 0) {
                        $(".empty").remove();
                        return $('.list-talent-product').append("<li class='empty'>没有更多了</li>");
                    }
                    $.each(result.data, function (index, value) {
                        $(".list-talent-product").append(
                            "<li class='item' onclick='getProduct( " + value['id'] + ")'>" +
                            "<img class='thumb' src='" + value['pic_url'] + "' style='background-size:cover;'>" +
                            "<span class='title'><font color='#1d709c'>" + value['title'] + "</font></span>" +
                            "<span class='original'>原价:" + value['price']['original_price'] + "元/" + value['unit'] + "</span>" +
                            "<span class='discount'>精品价:</span>" +
                            "<span class='price'>" + value['price']['now_price'] + "<span>元/" + value['unit'] + "</span></span>" +
                            "</li>");
                    });
                });
                npage += 6;
...
<script src="/Public/js/home/boutique.js" type="text/javascript" charset="utf-8"></script>
