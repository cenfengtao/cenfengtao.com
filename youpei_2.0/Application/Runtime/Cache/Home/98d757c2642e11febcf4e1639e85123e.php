...
<article class="middle">
    <div class="nav">
        <ul>
            ...
            <li class="on"><a href="<?php echo U('Orders/queryDeliveryByPage');?>" class="current" data-icon="" onclick="_hmt.push(['_trackEvent', 'footbar', 'click', '消息'])">待收货<i id="new_msg_alert"></i></a></li>
            <li><a href="<?php echo U('Orders/needCommentList');?>" onclick="_hmt.push(['_trackEvent', 'footbar', 'click', '消息'])">待评价<i id="new_msg_alert"></i></a></li>
        ...
        <ul class="list-order">
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><li>
                    ...
                    <?php if(!empty($val['group_record_id'])): ?><h3 class="title">
                        ...
                        <div class="sale"><?php endif; ?>
                    <?php if(!empty($val['product_id'])): ?><span class="checkExpress" onclick="checkExpress(<?php echo ($val['id']); ?>)">查看物流</span>
                    ...
                    <?php if(!empty($val['group_record_id'])): ?><span class="appeal" onclick="getGroup(<?php echo ($val['group_record_id']); ?>)">再次购买</span><?php endif; ?>
                    </div>
