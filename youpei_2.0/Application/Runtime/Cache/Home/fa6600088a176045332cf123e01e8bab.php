...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/Public/css/home/confirmationInfo.css"/>
    <style>
        .img img{
            width: 100%;
            height: 4.8rem;
...
<article class="middle">
    ...
    <section class="title">
        <h3 style="width: 100%;overflow: hidden;">史太姆积木课精选</h3>
    </section>
    ...
    <section class="roll">
        <span class="rolls"><strong>· </strong>优惠卷：</span>
        <strong>></strong>
        <span class="rollNum">-￥<span class="money">0</span></span>
    </section>
    ...
    <section class="use">
        说明：代金卷和优惠卷可同时使用，代金卷可同时选中多张使用，而优惠卷只能选中一张使用。
    </section>
    ...
    <section class="order">
        <span>原价：<strong>￥<span id="totalPrice">0</span></strong>/实付款：<strong>￥<span id="realPrice">0</span></strong></span>
    </section>
    <section class="buy">
...
<script>
    $(function () {
        var ajaxUrl = "/index.php/Parenting/ajaxConfirmationInfo";
        var par_id = getQueryString("par_id");
        var amount = getQueryString("amount");
        $.get(ajaxUrl, {par_id: par_id, amount: amount}, function (result) {
            if (result.status == 0) {
            ...
            var nowIntegralNum=parseFloat(nowIntegral/100);
                        ...
                        $(".coupon-list").append(
                                '<li class="coupon">'+
                                    '<label for="coupon-check-'+val.id+'">'+
                                        '<div class="content ">'+
                                            '<div class="texts">'+
                                                '<span>￥<strong>'+val.subtract.split(".")[0]+'</strong></span>'+
                                                '<div>'+
                                                    '<span>优惠卷</span>'+
                                                    '<p>订金满'+val.full.split(".")[0]+'元可使用</p>'+
                                                '</div>'+
                                            '</div>'+
                                            '<div class="hr"></div>'+
                                                '<p>有效期'+val.start_time +'-'+val.end_time+'</p>'+
                                        '</div>'+
                                        '<input type="radio" name="coupon" style="display: none;" class="coupon-checkbox" id="coupon-check-'+val.id+'" value="'+val.id+'" onclick="clickCoupon(' + val.id + ')">'+
                                        '<i class="check"></i>'+
                                    '</label>'+
        ...
        if (isChecked) { //选择当前
            ...
            if(integral){
		           if(totalPrice-nowPrice==totalPrice){
		                $(".money").text(parseInt(totalPrice-nowPrice))
                    }else{
                        $(".money").text(parseFloat(totalPrice-nowPrice).toFixed(2)-(integral * 0.01))
                    }
	            }else{
	            $(".money").text(totalPrice-nowPrice+(integral * 0.01))
	        }
        } else { //取消当前
    ...
    function addOrder() {
        ...
        var url = "/index.php/Orders/addParentingOrder";
        $.post(url, data, function (result) {
            if(result.status == 0 ){
                return alert(result.message);
            } else {
                window.location.href = "/index.php/WxPay/jsapiByParenting?orderId=" + result.orderId;
            }
        }, 'json')
    }
