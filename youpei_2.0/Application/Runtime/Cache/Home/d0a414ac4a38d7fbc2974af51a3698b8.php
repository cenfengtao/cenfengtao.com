<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html>
<!--headTrap<body></body><head></head><html></html>-->
<html>
<head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width,initial-scale=1,minimum-scale=1,maximum-scale=1,user-scalable=no" />
    <title>优培圈</title>
    <link rel="stylesheet" type="text/css" href="/Public/css/home/common.css"/>
    <link rel="stylesheet" type="text/css" href="/Public/css/home/com.css"/>
    <link rel="stylesheet" type="text/css" href="/Public/css/home/pageloader.css"/>
    <link rel="stylesheet" type="text/css" href="/Public/css/home/allOrders.css"/>
    <script type="text/javascript">
        if(/msie/i.test(navigator.userAgent)){window.location.href="http://www.youpei-exc.com/"}
    </script>
</head>
<body class="kap-loading ty-view-all">
<div id="bonfire-pageloader">
    <div class="bonfire-pageloader-icon">
       <svg class="kap-loading-icon" xmlns="http://www.w3.org/2000/svg"  xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="512px" height="512px"  enable-background="new 0 0 512 512" xml:space="preserve">
        <!--加载图片转码base64格式-->
        <g>
           <image class="ty-loading-icon" xlink:href="data:image/png;base64," id="ty_tree" height="50" width="50" y="0" x="0"/>
        </g>
        </svg>
    </div>  
</div>
<header class="header">
     <a href="javascript:history.back()" class="back">
        <img src="../../../../Public/images/back.png" alt="">
    </a>
    <h3><?php echo ($title); ?></h3>
    <a href="../Index/index.html" class="home" data-icon="Ņ"></a>
</div>
</header>
<article class="middle">
    <div class="nav">
        <ul>
            <li class="on"><a href="<?php echo U('Orders/allOrders');?>" class="current" >全部</a></li>
            <li><a href="<?php echo U('Orders/queryPayByPage');?>" onclick="_hmt.push(['_trackEvent', 'footbar', 'click', '发现'])">待付款</a></li>
            <li><a href="<?php echo U('Orders/queryDeliveryByPage');?>" onclick="_hmt.push(['_trackEvent', 'footbar', 'click', '消息'])">待收货<i id="new_msg_alert"></i></a></li>
            <li><a href="<?php echo U('Orders/needCommentList');?>" onclick="_hmt.push(['_trackEvent', 'footbar', 'click', '消息'])">已完成<i id="new_msg_alert"></i></a></li>
            <li><a href="<?php echo U('Orders/queryCancelOrders');?>" onclick="_hmt.push(['_trackEvent', 'footbar', 'click', '我的'])">已取消</a></li>
        </ul>
    </div>
    <div class="order-list">
        <ul class="list-order">
            <li>
            	<img src="../../../../Public/images/finished.png" alt="" class="finished"/>
                <h3 class="title">
                    <strong>优培教育</strong>
                    <span>></span>
                    <img src="../../../../Public/images/delete.png" alt="">
                </h3>
                <div class="content">
                    <img src="../../../../Public/images/school_product2.png" alt="">
                    <div class="text">
                        <h3>优培积木32课时优培积木32课时优培积木32课时优培积木32课时</h3>
                        <div class="type">
                            <span>2-8岁</span>
                            <span>动手</span>
                            <span>益智</span>
                        </div>
                        <p>
                            <span class="classTime">共32课时</span>
                            <span class="really">实付：<span class="sign">￥</span><strong>1800</strong></span>
                        </p>
                    </div>
                </div>
                <div class="time">
                    <span>下单时间：2017年10月30日 17:17:17</span>
                </div>
                <div class="sale">
                    <span>评价</span>
                    <span>申诉维权</span>
                    <span>再次购买</span>
                </div>
            </li>
            <!-- <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><li>
                    <?php if(!empty($val['product_id'])): ?><div class="type">
                            <span>优培教育 ></span>
                        </div>
                        <div class="left"><img src="<?php echo ($val['pro_pic']); ?>" onclick="getProduct(<?php echo ($val['product_id']); ?>)"></div>
                        <div class="rig"><p class="title" onclick="getProduct(<?php echo ($val['product_id']); ?>)"><?php echo ($val['pro_title']); ?></p></div><?php endif; ?>
                    <?php if(!empty($val['parenting_id'])): ?><div class="left"><img src="<?php echo ($val['pro_pic']); ?>" onclick="getParenting(<?php echo ($val['parenting_id']); ?>)"></div>
                        <div class="rig"><p class="title" onclick="getParenting(<?php echo ($val['parenting_id']); ?>)"><?php echo ($val['pro_title']); ?></p></div><?php endif; ?>
                    <?php if(!empty($val['activity_id'])): ?><div class="left"><img src="<?php echo ($val['pro_pic']); ?>" onclick="getActivity(<?php echo ($val['activity_id']); ?>)"></div>
                        <div class="rig"><p class="title" onclick="getActivity(<?php echo ($val['activity_id']); ?>)"><?php echo ($val['pro_title']); ?></p></div><?php endif; ?>
                    <?php if(!empty($val['group_record_id'])): ?><div class="left"><img src="<?php echo ($val['pro_pic']); ?>" onclick="getGroup(<?php echo ($val['group_record_id']); ?>)"></div>
                        <div class="rig"><p class="title" onclick="getGroup(<?php echo ($val['group_record_id']); ?>)"><?php echo ($val['pro_title']); ?></p></div> -->
                 <!--<?php endif; ?>
                    <p class="desc"><span class="price"><span>￥</span><?php echo ($val['price']); ?></span> &#160;<span>×<?php echo ($val['amount']); ?></span></p>
                    <p class="time"><?php echo (date("Y/m/d",$val['create_time'] )); ?> -->
                    <!-- 待付款 -->
                    <!-- <?php if($val['status'] == 0): ?><span class="quxiao" onclick="cancelOrder(<?php echo ($val['id']); ?>)">取消订单</span>
                            <span class="zhifu" onclick="payAgain(<?php echo ($val['id']); ?>)">重新支付</span><?php endif; ?> -->
                    <!-- 待收货 -->
                    <!-- <?php if($val['status'] == 1): if(!empty($val['product_id'])): ?><span class="checkExpress" onclick="checkExpress(<?php echo ($val['id']); ?>)">查看物流</span>
                            <span class="appeal" onclick="getProduct(<?php echo ($val['product_id']); ?>)">再次购买</span><?php endif; ?>
                        <?php if(!empty($val['parenting_id'])): ?><span class="appeal" onclick="getParenting(<?php echo ($val['parenting_id']); ?>)">再次购买</span><?php endif; ?>
                        <?php if(!empty($val['activity_id'])): ?><span class="appeal" onclick="getActivity(<?php echo ($val['activity_id']); ?>)">再次购买</span><?php endif; ?>
                        <?php if(!empty($val['group_record_id'])): ?><span class="appeal" onclick="getGroup(<?php echo ($val['group_record_id']); ?>)">再次购买</span><?php endif; endif; ?>
                    <?php if($val['status'] == 4): ?>-->
                        <!-- 待评价 -->
                       <!--  <?php if(!empty($val['isComment'])): ?><span class="checkExpress" onclick="appeal(<?php echo ($val['id']); ?>)">申诉订单</span>
                            <span class="appeal" onclick="addComment(<?php echo ($val['id']); ?>)">立即评价</span>
                            <?php else: ?>
                            <?php if(!empty($val['product_id'])): ?><span class="appeal" onclick="getProduct(<?php echo ($val['product_id']); ?>)">再次购买</span><?php endif; ?>
                            <?php if(!empty($val['parenting_id'])): ?><span class="appeal" onclick="getParenting(<?php echo ($val['parenting_id']); ?>)">再次购买</span><?php endif; ?>
                            <?php if(!empty($val['activity_id'])): ?><span class="appeal" onclick="getActivity(<?php echo ($val['activity_id']); ?>)">再次购买</span><?php endif; ?>
                            <?php if(!empty($val['group_record_id'])): ?><span class="appeal" onclick="getGroup(<?php echo ($val['group_record_id']); ?>)">再次购买</span><?php endif; endif; endif; ?> -->
                    <!-- 已取消 -->
                     <!--<?php if($val['status'] == 2): if(!empty($val['product_id'])): ?><span class="appeal" onclick="getProduct(<?php echo ($val['product_id']); ?>)">再次购买</span>
                            <span style="position:absolute;right: 1rem;color:#eb647e;">已取消</span><?php endif; ?>
                        <?php if(!empty($val['parenting_id'])): ?><span class="appeal" onclick="getParenting(<?php echo ($val['parenting_id']); ?>)">再次购买</span>
                            <span style="position:absolute;right: 1rem;color:#eb647e;">已取消</span><?php endif; ?>
                        <?php if(!empty($val['activity_id'])): ?><span class="appeal" onclick="getActivity(<?php echo ($val['activity_id']); ?>)">再次购买</span>
                            <span style="position:absolute;right: 1rem;color:#eb647e;">已取消</span><?php endif; ?>
                        <?php if(!empty($val['group_record_id'])): ?><span class="appeal" onclick="getGroup(<?php echo ($val['group_record_id']); ?>)">再次购买</span>
                            <span style="position:absolute;right: 1rem;color:#eb647e;">已取消</span><?php endif; endif; ?>
                    </p>
                </li><?php endforeach; endif; else: echo "" ;endif; ?>-->
            <li data-icon="Ĩ" class="item-more" style="display:none;">
                点击载入更多
            </li> 
        </ul>
    </div>
</article>
<script src="/Public/Home/js/jquery.min.js" type="text/javascript" charset="utf-8"></script>
<script src="/Public/Home/js/pageloader.js" type="text/javascript" charset="utf-8"></script>
<script src="/Public/js/home/rem.js" type="text/javascript" charset="utf-8"></script>
<!-- <script src="/Public/js/home/voteList.js" type="text/javascript" charset="utf-8"></script> -->
<script src="/Public/js/home/lazyload.js" type="text/javascript" charset="utf-8"></script>
<script>
$(function(){
        $(".nav li").click(function(){
            $(this).addClass("on").siblings().removeClass("on")
        })
        if($(".list-order li").length==1){
            $(".list-order").append('<p class="alert">你还没有商品哦，赶紧去选购吧！</p>')
        }
    });
    function appeal(id) {
        window.location.href = "<?php echo U('Orders/rejectOrder');?>?orderId="+id;
    }
    function addComment(id) {
        window.location.href = "<?php echo U('Orders/addComment');?>?id="+id;
    }
    function payAgain(id) {
        var url = "<?php echo U('Orders/payAgain');?>";
        $.get(url,{id:id},function (result) {
            if(result.status == 0){
                return alert(result.message);
            } else {
                switch(result.data['pro_type']){
                    case 'product':
                        window.location.href = "<?php echo U('WxPay/jsapiIndex');?>?orderId=" + result.data['order_id'];
                        break;
                    case 'parenting':
                        window.location.href = "/index.php/WxPay/jsapiByParenting?orderId=" + result.data['order_id'];
                        break;
                    case 'activity' :
                        window.location.href = "<?php echo U('WxPay/jsapiByActivity');?>?orderId="+ result.data['order_id'];
                        break;
                    case 'group' :
                        window.location.href = "<?php echo U('WxPay/jsapiByGroup');?>?orderId="+ result.data['order_id'];
                        break;
                    default :
                        return alert('订单错误');
                }
            }
        },'json');
    }
    function cancelOrder(id) {
        if(confirm("确认取消该订单？")){
            var url = "<?php echo U('Orders/cancelOrder');?>";
            $.post(url, {order_id: id}, function (result) {
                if (result.status == 0) {
                    return alert(result.msg);
                } else {
                    location.reload();
                }
            }, 'json');
        }
    }
    function deleteOrder(id) {
        var url = "<?php echo U('Orders/deleteOrder');?>";
        $.post(url, {order_id: id}, function (result) {
            if (result.status == 0) {
                return alert(result.msg);
            }
            if (result.status == 1) {
                alert(result.msg);
                location.reload();
            }
        }, 'json');
    }
    function getProduct(id) {
        window.location.href = "<?php echo U('Product/productDetails');?>?pro_id="+id;
    }

    function getParenting(id) {
        window.location.href = "<?php echo U('Parenting/productDetails');?>?par_id="+id;
    }

    function getActivity(id) {
        window.location.href = "<?php echo U('Organization/activityDetail');?>?id="+id;
    }

    function getGroup(id) {
        window.location.href = "<?php echo U('Groups/getGroup');?>?id="+id;
    }

    function addComment(id) {
        window.location.href = "/index.php/Orders/addComment?id="+id;
    }

    function appeal(id) {
        window.location.href = "<?php echo U('Orders/rejectOrder');?>?orderId="+id;
    }

    function checkExpress(id) {
        window.location.href = "/index.php/Orders/checkExpress?id="+id;
    }
</script>
</body>
</html>
<!--tailTrap<body></body><head></head><html></html>-->