...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/Public/css/homes/voteList.css"/>
...
<header class="header">
    <a href="/index.php/Votes/voteActivityList.html" class="back" style="width: 2.2rem;">
...
<article class="middle">
    ...
    <section class="organization">
        <div id="youpei" style="display: none;">
             <span>主办方：</span>
             <a href=""><img src="" alt="" class="youpei"></a>
        </div>
       <div id="help" style="display: none;">
            <span>协办方：</span>
            <a href="" class="orgIndex"><img src="" alt="" class="help"></a>
       </div>
...
<script src="/Public/js/homes/voteList.js" type="text/javascript" charset="utf-8"></script>
...
$(".userBillboard").attr("href","/index.php/Votes/userBillboard.html?vote_id="+vote_id)
 // 微信端分享
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            if(data.status==1){
                ...
                if(data.data.organizer.length==0){
                    $("#help").hide()
                }else{
                    $("#help").show().find(".help").attr("src",data.data.organizer.picture)
                    $("#help").find("a").attr("href","/index.php/Organization/home.html?id="+data.data.organizer.id)
                }
                if(data.data.sponsor==null){
                    $("#youpei").hide()
                }else{
                    $("#youpei").show().find(".youpei").attr("src",data.data.sponsor.picture)
                    $("#youpei").find("a").attr("href","/index.php/Organization/home.html?id="+data.data.sponsor.id)
                }
                             ...
                             $("#content").append(
                                '<li onclick="toVote('+joinUser[i].id+')" id="'+joinUser[i].id+'" class="box">'+
                                    '<div class="img">'+
                                         ...
                                         '<span class="order">'+joinUser[i].number+'号</span>'+
                        ...
                        for(var j=0;j<joinUserLength; j++){
                             $("#content").append(
                                '<li onclick="toVote('+joinUser[j].id+')" id="'+joinUser[j].id+'" class="box">'+
                                    '<div class="img">'+
                                         '<img src="'+joinUser[j].path+'" />'+
                                          '<img class="greenTap" src="../../../../Public/images/greenTap.png"/>'+
                                         '<span class="order">'+joinUser[j].number+'号</span>'+
                                         // '<span>'+joinUser[i].id+'</span>'+
                                    '</div>'+
    ...
    function toVote(id){
        window.location.href="/index.php/Votes/voteDetail.html?id="+id+"&vote_id="+vote_id;
    }
    function join(){
         window.location.href="/index.php/Votes/contribution.html?vote_id="+vote_id;
    }
    function end(){
