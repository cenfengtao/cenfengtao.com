...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/Public/css/home/userWorks.css"/>
...
<script src="/Public/js/home/userWorks.js" type="text/javascript" charset="utf-8"></script>
...
<script>
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            var image = $('.swiper-wrapper img')[0].src;
            var id = $('#id').val();
            if($('#status').val() == 2){
                var f_title='你的好友'+username+'邀请你来帮他投票！';
                var shareUrl='http://'+window.location.host+'/index.php/Vote/voteDetail.html?id='+id+'&vote_id='+vote_id+'&token=g232238gc959';
                var shareImg=image;
            }else{
                var f_title='你的好友'+username+'邀请你去参加活动！';
                var shareUrl='http://'+window.location.host+'/index.php/Vote/contribution.html?vote_id='+vote_id+'&token=g232238gc959';
                var shareImg=data.data.share_img;
            }
            var title='【优培·投票】 '+data.data.title;
            // 调用微信的分享接口
            wx.config({
