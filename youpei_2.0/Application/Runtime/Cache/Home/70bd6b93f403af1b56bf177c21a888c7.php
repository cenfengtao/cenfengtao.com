<?php if (!defined('THINK_PATH')) exit();?><!doctype html>
<!--headTrap<body></body><head></head><html></html>--><html>
<head>
<meta charset="utf-8">
<title>优培圈</title>
<meta name="viewport" content="width=device-width, initial-scale=0.5, user-scalable=0, minimum-scale=0.5, maximum-scale=0.5">
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1" /><!-- iOS webapp s-->
<meta name="format-detection" content="telephone=no" />
<meta name="apple-mobile-web-app-capable" content="yes">
<meta name="keywords" content="优培圈" />
<meta name="description" content="" />
<link rel="apple-touch-icon-precomposed" href="styles/app_icon.png">
<!-- iOS webapp e -->
<link href="/Public/Home/css/kap.css" rel="stylesheet" type="text/css">
<link href="/Public/Home/css/common.css" rel="stylesheet" type="text/css">
<link href="/Public/Home/css/index.css" rel="stylesheet" type="text/css">
<link rel="stylesheet" type="text/css" href="/Public/index/member/css/dialog.css" media="all" />
<script type="text/javascript" src="/Public/Home/js/jweixin-1.0.0.js"></script>
<script type="text/javascript" src="/Public/index/member/js/dialog.js"></script>
<!--移动端兼容适配 end -->
<script src="/Public/Home/js/jquery-1.7.1.min.js"></script>
<script type="text/javascript">
if(/msie/i.test(navigator.userAgent)){window.location.href="http://www.youpei-exc.com/"}
</script>
<script>
var _hmt = _hmt || [];
(function() {
  var hm = document.createElement("script");
  hm.src = "https://hm.baidu.com/hm.js?548e13b07cb36e38bfd73870368a0253";
  var s = document.getElementsByTagName("script")[0]; 
  s.parentNode.insertBefore(hm, s);
})();


// var ua = navigator.userAgent.toLowerCase();
//     if(ua.indexOf("iphone")>0){ //app

//         get_ios_script();
//     }else{
//         get_android_script();
//         $("meta[name=viewport]").attr('content', 'width=device-width, initial-scale=1.0, user-scalable=0, minimum-scale=1.0, maximum-scale=1.0'); 
//     }
//     function get_ios_script() {
//       var link = document.createElement('link');
//         link.type = 'text/css';
//         link.rel = 'stylesheet';
//         link.href = '/Public/Home/css/ios.css';
//         document.getElementsByTagName("head")[0].appendChild(link);
//     }

  
</script>
</head>
<link href="/Public/Home/css/pageloader.css" rel="stylesheet" type="text/css">
<body class="kap-loading ty-view-all ty-ios" onload="">
<div class="kap-wrap">
<!--加载 -->
<div id="bonfire-pageloader">
    <div class="bonfire-pageloader-icon">
<svg class="kap-loading-icon" xmlns="http://www.w3.org/2000/svg"  xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="512px" height="512px"  enable-background="new 0 0 512 512" xml:space="preserve">
 <!-- Created with SVG-edit - http://svg-edit.googlecode.com/ -->
 <!--加载图片转码base64格式-->
<g>
 <image class="ty-loading-icon" xlink:href="data:image/png;base64," id="ty_tree" height="100" width="100" y="0" x="0"/>
 </g>
 </svg>
</div>
</div>

<link rel="stylesheet" href="/Public/Home/css/idangerous.swiper.css">
<link rel="stylesheet" href="/Public/Home/css/style_v3.css">
<link rel="stylesheet" href="/Public/Home/css/new.css">
<link rel="stylesheet" href="/Public/css/home/organizationHome.css">
<style>
    .list-card .thumb img{
        height: 11rem;
    }
    .list-card .thumb{
        height: 11rem!important;
        padding-bottom: 0;
    }
</style>
<div id="page_user" title="<?php echo ($title); ?>" class="kap-page kap-page-current">
    <div class="kap-top ty-menu-top">
        <div id="tools_msg" class="ty-tools ty-tools-inner ty-tools-current">
            <span class="title"><?php echo ($title); ?><span class="num"></span></span>
            <a href="javascript:history.go(-1);" class="btn left btn-user" title="返回" data-icon="ɒ"></a>
            <a href="<?php echo U('Index/index',array('current'=>'1'));?>" class="btn right btn-user" title="返回首页"
               data-icon="Ņ"></a>
        </div>
    </div>
    <div class="advert">
        <!-- <a href="###"><img src="../../../../Public/images/article-banner.png" alt=""></a> -->
    </div>
    <div class="ty-page" data-ty-current="0" data-ty-total="0">
        <div class="shop-info">
            <div class="shoplogo">
                <img src="<?php echo ($organization['picture']); ?>">
                <span><?php echo ($organization['org_name']); ?></span>
            </div>
            <div class="score">
                <span>机构评级</span>
                 <span class="star" data-ty-talent-star="5">
                    <?php $__FOR_START_12668__=0;$__FOR_END_12668__=$organization['org_star'];for($$organization['org_star']=$__FOR_START_12668__;$$organization['org_star'] < $__FOR_END_12668__;$$organization['org_star']+=1){ ?><i class="on iconfont">Ŧ</i><?php } ?>
                    </span>
                <span>环境评级</span>
                 <span class="star" data-ty-talent-star="5">
                  <?php $__FOR_START_9335__=0;$__FOR_END_9335__=$organization['env_star'];for($$organization['env_star']=$__FOR_START_9335__;$$organization['env_star'] < $__FOR_END_9335__;$$organization['env_star']+=1){ ?><i class="on iconfont">Ŧ</i><?php } ?>
                    </span>
                <span>教学质量</span>
                 <span class="star" data-ty-talent-star="5">
                     <?php $__FOR_START_11265__=0;$__FOR_END_11265__=$organization['quality_star'];for($$organization['quality_star']=$__FOR_START_11265__;$$organization['quality_star'] < $__FOR_END_11265__;$$organization['quality_star']+=1){ ?><i class="on iconfont">Ŧ</i><?php } ?>
                    </span>
            </div>
            <div class="details">
                <span onclick="getIntroduce(<?php echo ($organization['id']); ?>)">了解TA</span>
                <span id="f0_txt" onclick="followed(<?php echo ($organization['id']); ?>)"><?php if($isFollowed == 1): ?>已关注
                    <?php else: ?>
                    关注<?php endif; ?></span></div>
            <label>关注可获取TA的教学动态噢！</label>
            <div class="choice">
                <div class="choice-classify">
                    <div class="swiper-container swiper-nested-2">
                        <div class="swiper-wrapper">
                            <?php if(is_array($banners)): foreach($banners as $key=>$val): if($val['type'] == 1): ?><div class="swiper-slide">
                                        <a href="/index.php/Article/getArticle.html?art_id=<?php echo ($val['type_id']); ?>"><img
                                                src="<?php echo ($val['image']); ?>"></a>
                                    </div>
                                    <?php else: ?>
                                    <div class="swiper-slide">
                                        <a href="/index.php/Product/productDetails.html?pro_id=<?php echo ($val['type_id']); ?>"><img
                                                src="<?php echo ($val['image']); ?>"></a>
                                    </div><?php endif; endforeach; endif; ?>
                        </div>
                    </div>
                    <div class="pagination pagination-nested-2"></div>
                </div>
            </div>
            <?php if(!empty($offerList)): ?><div class="coupon">
                    <a id="getOffer" onclick="getOffer()">立即领取课程优惠券</a>
                </div><?php endif; ?>
        </div>
        <div class="act-tag">
            <a href="#desc" class="tag detail on" attr_id="1" onclick="tabs(0)">热门课程</a>
            <a href="#cost" class="tag price" attr_id="2" onclick="tabs(1)">文章·福利</a>
            <a href="#comment" class="tag qa" attr_id="3" onclick="tabs(2)">活动安排</a>
        </div>
        <input type="hidden" name="id" id="id" value="<?php echo ($organization['id']); ?>">
        <div class="tabcon" style="display: block;">
            <ul class="list-card">
                <?php if(is_array($classList)): foreach($classList as $key=>$val): ?><li class="item">
                        <a href="/index.php/Product/productDetails.html?pro_id=<?php echo ($val['id']); ?>">
        <span class="thumb" style=""><img class="lazy" data-original="<?php echo ($val['pic_url']); ?>">
        </span>
                            <div class="desc">
                                <span data-icon="Ć" class="loc">
                                    <?php echo ($organization['province']); echo ($organization['city']); echo ($organization['area']); echo ($organization['address']); ?>
                                </span>
                                <div class="triangle">
                                    <p class="text"><?php echo ($val['class_title']); ?></p>
                                </div>
                                <h3 class="title"><?php echo ($val['title']); ?></h3>
                                <span class="price"><?php echo ($val['price']['now_price']); ?></span>
                            </div>
                        </a></li><?php endforeach; endif; ?>
            </ul>
            <ul class="hotart emptyone list-card"></ul>
            <div class="logo-qrcode">
                <span><img src="<?php echo ($organization['picture']); ?>" style="height: auto;"></span>
		     <span><img src="<?php echo ($organization['qr_code']); ?>">
		     <br/><span>扫一扫关注我们</span></span>
            </div>
        </div>
        <div class="tabcon" style="display: none;">
            <div class="weffimg">
                <div class="swiper-container swiper-nested-1">
                    <div class="swiper-slide">
                        <a href="/index.php/index/index.html"> <img src="/Public/images/fffffff.png"></a>
                    </div>
                </div>
                <div class="pagination pagination-nested-1"></div>
            </div>
            <!--<div class="item-more shareline"> —— 分享领红包 ——</div>-->
            <div class="timeline-product shoparticle">
                <ul class="list-card-timeline">
                    <?php if(is_array($articleList)): foreach($articleList as $key=>$val): ?><li class="item" data-ty-target="#detail_activity" onclick="getArticle(<?php echo ($val['id']); ?>)">
                        <span class="thumb"
                              style="background-image:url(<?php echo ($val['image']); ?>); background-size:cover"></span>
                        <span class="desc">
                            <h3 class="title"><?php echo ($val['title']); ?></h3>
                            <span class="time"><span class="lef"><?php echo ($val['time']); ?></span><span class="rig">评论（<?php echo ($val['comment_count']); ?>） <i></i> 收藏（<?php echo ($val['collect']); ?>）</span></span>
                            <span class="status dt"><?php echo ($val['cate_title']); ?></span>
                        </span>
                            <span class="icon-sale"></span>
                        </li><?php endforeach; endif; ?>
                </ul>
                <ul class="article-boon list-card-timeline">
                </ul>
            </div>

            <div class="item-more shareline"> —— 更多精彩推荐 ——</div>
            <div class="more-article">
                <ul>
                    <?php if(is_array($newList)): $i = 0; $__LIST__ = $newList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><li onclick="getArticle(<?php echo ($val['id']); ?>)"><img src="<?php echo ($val['org_pic']); ?>"> <span><?php echo ($val['title_name']); ?>...</span><label
                                data-icon="Ĩ"> </label></li><?php endforeach; endif; else: echo "" ;endif; ?>
                </ul>
            </div>
            <div>
            </div>
        </div>
        <div class="tabcon line-height" style="display: none;">
            <div class="weffimg">
                <div class="item-more shareline"> —— 赶快报名吧 ——</div>
                <ul class="list-card">
                    <?php if(is_array($activityList)): foreach($activityList as $key=>$val): ?><li onclick="" style="float: none; display: block; position: relative; top: 0px; left: 0px;">
                            <a href="<?php echo U('Organization/activityDetail');?>?id=<?php echo ($val['id']); ?>">
                                    <span class="thumb"
                                          style="background-image:url(<?php echo ($val['image']); ?>); background-size:cover">
                                    </span>
                                <span class="status"><p data-icon-fiery="ə">进行中</p></span>
                                <div class="desc row">
                                    <label class="size red"
                                           style="color:#ffffff;margin-right:3vh;">活动时间：<?php echo (date("Y-m-d",$val['activity_time'])); ?></label>
                                </div>
                            </a>
                        </li><?php endforeach; endif; ?>
                    <div class="activity"></div>
                </ul>
                <div class="item-more shareline"> —— 更多精彩活动 ——</div>
                <div class="more-article">
                    <ul>
                        <?php if(is_array($actList)): $i = 0; $__LIST__ = $actList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><li onclick="getActivity(<?php echo ($val['id']); ?>)"><img src="<?php echo ($val['org_pic']); ?>"> <span><?php echo ($val['title_name']); ?>...</span>
                                <label data-icon-go="Ĩ">去参加</label></li><?php endforeach; endif; else: echo "" ;endif; ?>
                    </ul>
                </div>
            </div>
        </div>
        <!-- 底部菜单 -->
        <div class="kap-bottom ty-menu-bottom">
            <div id="tools_idx" class="ty-tools ty-tools-idx ty-tools-current">
                <a href="" class="current" data-icon="Ņ">TA首页</a>
                <a href="<?php echo U('Organization/fullShot');?>?id=<?php echo ($organization['id']); ?>"
                <?php if($current == 1): ?>class="current"<?php endif; ?>
                data-icon="Đ">全景</a>
                <a href="<?php echo U('Association/index',array('current'=>'3'));?>"
                <?php if($current == 3): ?>class="current"<?php endif; ?>
                data-icon="Ĵ">社群</a>
                <a href=""
                <?php if($current == 1): ?>class="current"<?php endif; ?>
                data-icon="Ē">客服</a>
            </div>
        </div>
    </div>
    <input type="hidden" name="is_show" value="<?php echo ($isShow); ?>">
    <script src="/Public/Home/js/jquery.min.js"></script>
<?php if($umark == "productDetails"): ?><div id="side_article" class="ty-sidebar-area ty-tools ty-tools-current">
        <div class="section">
            <span class="qrcode"><img src="/Public/images/618348288222690487.jpg" alt="" height="166" width="166"><b
                    class="tip">微信扫一扫</b></span>
            <p>关注“<b>优培圈</b>”微信公众号， “<b class="wx-code">立即购买</b></p>
            <span class="btn phone" data-icon="Ĉ">客服电话<b>1569357635</b></span>
        </div>
        <p class="copyright">
            Copyright &copy;2017 youpei<br>
            <a href="#" target="_blank">粤ICP备17008351号</a>
        </p>
    </div><?php endif; ?>


<!-- 20160621 updated -->
<script language="javascript">
    function getUrlArg(name) {
        var reg = new RegExp("(^|&)" + name + "=([^&]*)(&|$)");
        var r = window.location.search.substr(1).match(reg);
        if (r != null)return unescape(r[2]);
        return null;
    }
    function is_weixn() {
        var ua = navigator.userAgent.toLowerCase();
        if (ua.match(/MicroMessenger/i) == "micromessenger") {
            return true;
        } else {
            return false;
        }
    }
    function setCookie(name, value) {
        var Days = 30;
        var exp = new Date();
        exp.setTime(exp.getTime() + Days * 24 * 60 * 60 * 1000);
        document.cookie = name + "=" + escape(value) + ";expires=" + exp.toGMTString();
    }

    function delCookie(c_name, value) {
        var date = new Date();
        //将date设置为过去的时间
        date.setTime(date.getTime() - 10000);
        //将userId这个cookie删除
        document.cookie = c_name + "=" + escape(value) + " ; expires=" + date.toGMTString();
    }

    function getCookie(name) {
        var arr = document.cookie.match(new RegExp("(^| )" + name + "=([^;]*)(;|$)"));
        if (arr != null) return unescape(arr[2]);
        return null;
    }
    var timestamp = (new Date()).valueOf();
    var hash = location.hash;
    var source = getUrlArg("source");
</script>
<script src="/Public/Home/js/jweixin-1.0.0.js"></script>
<script src="/Public/Home/js/jquery.ba-hashchange.min.js"></script>
<script src="/Public/Home/js/jquery.exif.min.js"></script>
<script type="text/vbscript">
Function IEBinary_getByteAt(strBinary, iOffset)
    IEBinary_getByteAt = AscB(MidB(strBinary,iOffset+1,1))
End Function
Function IEBinary_getLength(strBinary)
    IEBinary_getLength = LenB(strBinary)
End Function
</script>
<script src="/Public/Home/js/kap.js?v6.6"></script>
<script src="/Public/Home/js/common.js?v6.6"></script>
<script src="/Public/Home/js/c.js?v6.6"></script>
<script src="/Public/Home/layer/layer.js"></script>
<script language="javascript">
    function handleOpenURL(url) {
        setTimeout(function () {
            var link = "";
            if (link.indexOf("http://www.youpei-exc.com") == 0) window.location.href = link;
        }, 0);
    }
</script>
<!--<script src="/Public/Home/js/selectivizr.js?v1.3"></script>-->
<!--加载 -->
<script src="/Public/Home/js/pageloader.js"></script>
<script>
    jQuery(window).resize(function () {
        resizenow();
    });
    function resizenow() {
        var browserwidth = jQuery(window).width();
        var browserheight = jQuery(window).height();
        jQuery('.bonfire-pageloader-icon').css('right', ((browserwidth - jQuery(".bonfire-pageloader-icon").width()) / 2)).css('top', ((browserheight - jQuery(".bonfire-pageloader-icon").height()) / 2));
    };
    resizenow();
</script>
<script>
    wx.config({
        debug: false, // 开启调试模式,调用的所有api的返回值会在客户端alert出来，若要查看传入的参数，可以在pc端打开，参数信息会通过log打出，仅在pc端时才会打印。
        appId: "<?php echo ($signPackage['appId']); ?>", // 必填，公众号的唯一标识
        timestamp: "<?php echo ($signPackage['timestamp']); ?>", // 必填，生成签名的时间戳
        nonceStr: "<?php echo ($signPackage['nonceStr']); ?>", // 必填，生成签名的随机串
        signature: "<?php echo ($signPackage['signature']); ?>",// 必填，签名，见附录1
        jsApiList: ['onMenuShareTimeline', 'onMenuShareAppMessage', 'onMenuShareQQ', 'onMenuShareWeibo', 'hideOptionMenu', 'showOptionMenu', 'hideMenuItems', 'showMenuItems', 'hideAllNonBaseMenuItem', 'showAllNonBaseMenuItem', 'closeWindow', 'chooseImage', 'uploadImage', 'getLocation'] // 必填，需要使用的JS接口列表，所有JS接口列表见附录2
    });
    wx.ready(function () {
        wx.onMenuShareAppMessage({
            title: "<?php echo ($shareData['share_title']); ?>",
            desc: "<?php echo ($shareData['share_desc']); ?>",
            link: "<?php echo ($shareData['share_url']); ?>",
            imgUrl: "<?php echo ($shareData['share_img']); ?>",
            success: function () {
                alert('分享成功');
            },
            cancel: function () {
                alert('已取消');
            }
        });
        wx.onMenuShareTimeline({
            title: "<?php echo ($shareData['share_title']); ?>",
            link: "<?php echo ($shareData['share_url']); ?>",
            imgUrl: "<?php echo ($shareData['share_img']); ?>",
            success: function () {
                alert('分享成功');// 用户确认分享后执行的回调函数
            },
            cancel: function () {
                alert('已取消');// 用户取消分享后执行的回调函数
            }
        });
        wx.error(function (res) {
//            alert(JSON.stringify(res));
        });
    });
</script>
</div>

    <script src="/Public/Home/js/idangerous.swiper.min.js"></script>
    <script src="/Public/Home/js/jquery.lazyload.js?v=1.9.1"></script>
    <script src="/Public/js/layer/mobile/layer.js"></script>
    <script type="text/javascript">
        var isShow = $("input[name='is_show']").val();
        if(isShow == 2){
            alert('该机构暂未开放');
            setTimeout(function () {
                window.history.back();
            },2000)
        }
        var itemL = $(".list-card-timeline li").length;
        for (var i = 0; i < itemL; i++) {
            var color = document.getElementsByClassName("dt")[i].innerHTML;
            if (color == "视频") {
                document.getElementsByClassName("dt")[i].style.backgroundColor = "#ea775b";
            }
            if (color == "育儿") {
                document.getElementsByClassName("dt")[i].style.backgroundColor = "#f59600";
            }
            if (color == "亲子") {
                document.getElementsByClassName("dt")[i].style.backgroundColor = "#19f724";
            }
            if (color == "故事") {
                document.getElementsByClassName("dt")[i].style.backgroundColor = "#1dd2a1";
            }
            if (color == "实验") {
                document.getElementsByClassName("dt")[i].style.backgroundColor = "#3176c3";
            }
            if (color == "绘本") {
                document.getElementsByClassName("dt")[i].style.backgroundColor = "#9fc4e0";
            }
            if (color == "福利") {
                document.getElementsByClassName("dt")[i].style.backgroundColor = "#ff84b0";
            }
        }
        var itemLe = $(".list-card li").length;
        for (var j = 0; j < itemLe; j++) {
            var color = document.getElementsByClassName("text")[j].innerHTML;
//			console.log(color)
//			console.log(color=="视频")
            if (color == "视频") {
//				console.log(1111)
                document.getElementsByClassName("text")[j].style.backgroundColor = "#ea775b";
            }
            if (color == "早教") {
                document.getElementsByClassName("triangle")[j].style.borderColor = "#f59600";
            }
            if (color == "画画") {
                document.getElementsByClassName("triangle")[j].style.borderColor = "#19f724";
            }
            if (color == "积木") {
                document.getElementsByClassName("triangle")[j].style.borderColor = "#1dd2a1";
            }
            if (color == "实验") {
                document.getElementsByClassName("text")[j].style.backgroundColor = "#3176c3";
            }
            if (color == "绘本") {
                document.getElementsByClassName("text")[j].style.backgroundColor = "#9fc4e0";
            }
            if (color == "福利") {
                document.getElementsByClassName("text")[j].style.backgroundColor = "#ff84b0";
            }
        }

        $(function () {

            $("img.lazy").lazyload({effect: "fadeIn"});

        });
        $(window).load(function () {
            setTimeout(function () {
                $('.kap-bottom').show();
            }, 2000)
        });
        function tabs(tabNum) {
            //设置点击后的切换样式
            $(".act-tag a").removeClass("on");
            $(".act-tag a").eq(tabNum).addClass("on");
            //根据参数决定显示内容
            $(".tabcon").hide();
            $(".tabcon").eq(tabNum).show();

        }
        var swiperParent = new Swiper('.swiper-parent', {
            pagination: '.pagination-parent',
            loop: true,
            grabCursor: true,
            paginationClickable: true,
            autoplay: 2500,
            slidesPerView: 1
        });
        var swiperNested2 = new Swiper('.swiper-nested-2', {
            pagination: '.pagination-nested-2',
            loop: true,
            grabCursor: true,
            paginationClickable: true,
            autoplay: 2500,
            slidesPerView: 1,
            autoplayDisableOnInteraction: false,
        });

        var page = 3;
        $(function () {
            $(window).scroll(function () {
                var scrollTop = $(this).scrollTop();
                var scrollHeight = $(document).height();
                var windowHeight = $(this).height();
                var cate_id = $('.act-tag .on').attr('attr_id');
                var id = $('#id').val();
                if (scrollTop + windowHeight == scrollHeight) {
                    //加载层
                    layer.load();
                    setTimeout(function () {
                        layer.closeAll('loading');
                    }, 1000);
                    var url = "<?php echo U('Organization/loadingOrganization');?>";
                    var params = {};
                    params.cate_id = cate_id;
                    params.id = id;
                    params.page = page;
                    $.post(url, params, function (result) {
                        if (result.status == 1) {
                            $(".empty").remove();
                            return $('.emptyone').append("<li class='empty' style='line-height:3rem;'>没有更多了</li>");
                        } else if (result.status == 2) {
                            $.each(result.data, function (index, value) {
                                $(".hotart").append("<li class='item'>" +
                                        "<a href='/index.php/Product/productDetails.html?pro_id=" + value['id'] + "'>" +
                                        "<span class='thumb'><img src='" + value['pic_url'] + "' alt=''></span>" +
                                        "<div class='desc'>" +
                                        "<span data-icon='Ć' class='loc'>" + value['province'] + value['city'] + value['area'] + value['address'] + "</span>" +
                                        "<div class='triangle'><p class='text'>" + value['class_title'] + "</p></div>" +
                                        "<h3 class='title'>" + value['title'] + "</h3>" +
                                        "<span class='price'>" + value['price'] + "</span>" +
                                        "</div>" +
                                        "</a>" +
                                        "</li>");
                            });
                        } else if (result.status == 3) {
                            $.each(result.data, function (index, value) {
                                $(".article-boon").append("<li class='item' onclick='getArticle( " + value['id'] + ")'>" +
                                        "<img class='thumb' src='" + value['image'] + "' style='background-size:cover;'>" +
                                        "<span class='desc'>" +
                                        "<h3 class='title'>" + value['title'] + "</h3>" +
                                        "<span class='time'><span class='lef'>" + value['time'] + "</span>" +
                                        "<span class='rig'>评论（" + value['comment_count'] + "） <i></i> 收藏（" + value['collect'] + "）</span></span>" +
                                        "<span class='status dt'>" + value['cate_title'] + "</span>" +
                                        "</span>" +
                                        "<span class='icon-sale'></span>" +
                                        "</li>");
                            });
                        } else if (result.status == 4) {
                            $.each(result.data, function (index, value) {
                                $(".activity").append(
                                        "<li style='float: none; display: block; position: relative; top: 0px; left: 0px;'>" +
                                        "<a href='<?php echo U('Organization/activityDetail');?>?id=" + value['id'] + "'>" +
                                        "<span class='thumb' style='background-image:url(" + value['image'] + "); background-size:cover'></span>" +
                                        "<span class='status'><p data-icon-fiery='ə'>进行中</p></span>" +
                                        "<div class='desc row'><label class='size red' style='color:#ffffff;margin-right:3vh;'>活动时间：" + value['time'] + "</label></div>" +
                                        "</a>" +
                                        "</li>");
                            });
                        }

                        var itemL = $(".list-card-timeline li").length;
                        for (var i = 0; i < itemL; i++) {
                            var color = document.getElementsByClassName("dt")[i].innerHTML;
                            //			console.log(color)
                            //			console.log(color=="视频")
                            if (color == "视频") {
                                //				console.log(1111)
                                document.getElementsByClassName("dt")[i].style.backgroundColor = "#ea775b";
                            }
                            if (color == "育儿") {
                                document.getElementsByClassName("dt")[i].style.backgroundColor = "#f59600";
                            }
                            if (color == "亲子") {
                                document.getElementsByClassName("dt")[i].style.backgroundColor = "#19f724";
                            }
                            if (color == "故事") {
                                document.getElementsByClassName("dt")[i].style.backgroundColor = "#1dd2a1";
                            }
                            if (color == "实验") {
                                document.getElementsByClassName("dt")[i].style.backgroundColor = "#3176c3";
                            }
                            if (color == "绘本") {
                                document.getElementsByClassName("dt")[i].style.backgroundColor = "#9fc4e0";
                            }
                            if (color == "福利") {
                                document.getElementsByClassName("dt")[i].style.backgroundColor = "#ff84b0";
                            }
                        }


                        var itemLe = $(".list-card li").length;
                        for (var j = 0; j < itemLe; j++) {
                            var color = document.getElementsByClassName("text")[j].innerHTML;
                            //			console.log(color)
                            //			console.log(color=="视频")
                            if (color == "视频") {
                                //				console.log(1111)
                                document.getElementsByClassName("text")[j].style.backgroundColor = "#ea775b";
                            }
                            if (color == "早教") {
                                document.getElementsByClassName("triangle")[j].style.borderColor = "#f59600";
                            }
                            if (color == "画画") {
                                document.getElementsByClassName("triangle")[j].style.borderColor = "#19f724";
                            }
                            if (color == "积木") {
                                document.getElementsByClassName("triangle")[j].style.borderColor = "#1dd2a1";
                            }
                            if (color == "实验") {
                                document.getElementsByClassName("text")[j].style.backgroundColor = "#3176c3";
                            }
                            if (color == "绘本") {
                                document.getElementsByClassName("text")[j].style.backgroundColor = "#9fc4e0";
                            }
                            if (color == "福利") {
                                document.getElementsByClassName("text")[j].style.backgroundColor = "#ff84b0";
                            }
                        }
                    });
                    page += 3;


                    var itemL = $(".list-card-timeline li").length;
                    for (var i = 0; i < itemL; i++) {
                        var color = document.getElementsByClassName("dt")[i].innerHTML;
                        //			console.log(color)
                        //			console.log(color=="视频")
                        if (color == "视频") {
                            //				console.log(1111)
                            document.getElementsByClassName("dt")[i].style.backgroundColor = "#ea775b";
                        }
                        if (color == "育儿") {
                            document.getElementsByClassName("dt")[i].style.backgroundColor = "#f59600";
                        }
                        if (color == "亲子") {
                            document.getElementsByClassName("dt")[i].style.backgroundColor = "#19f724";
                        }
                        if (color == "故事") {
                            document.getElementsByClassName("dt")[i].style.backgroundColor = "#1dd2a1";
                        }
                        if (color == "实验") {
                            document.getElementsByClassName("dt")[i].style.backgroundColor = "#3176c3";
                        }
                        if (color == "绘本") {
                            document.getElementsByClassName("dt")[i].style.backgroundColor = "#9fc4e0";
                        }
                        if (color == "福利") {
                            document.getElementsByClassName("dt")[i].style.backgroundColor = "#ff84b0";
                        }
                    }
                }
            });
        });


        function getInfo(id) {
            window.location.href = "<?php echo U('Organization/home');?>?id=" + id;
        }
        function getArticle(id) {
            window.location.href = "<?php echo U('Article/getArticle');?>?art_id=" + id;
        }
        function getIntroduce(id) {
            window.location.href = "<?php echo U('Organization/getOrganizationIntroduce');?>?id=" + id;
        }
        function getActivity(id) {
            window.location.href = "<?php echo U('Organization/activityDetail');?>?id=" + id;
        }

        function followed(id) {
            var url = "<?php echo U('Organization/followed');?>";
            $.post(url, {id: id}, function (res) {
                if (res.status == 1) {
                    alert('成功关注');
                    $('#f0_txt').text('已关注');
                } else if (res.status == 2) {
                    alert('成功取消关注');
                    $('#f0_txt').text('关注');
                }
            }, 'json')
        }
        // $.ajax({
        //  type: "post",
        //  url: "/index.php/Activity/indexAjax",
        //  aysnc: true,
        //  dataType: "json",
        //  success:function(data){
        //      //遍历文章或视频数据
        //     for(var a=0; a<articleLength; a++){
        //         var articleId=data.data.articles[a].id;
        //         var articleImg=data.data.articles[a].image;
        //         var article=data.data.articles[a];
        //         $(".articles").append(
        //              '<li class="articleLi" onclick="getArticle('+articleId+')">'+
        //                ' <img src="'+articleImg+'" alt="">'+
        //                 '<div class="text">'+
        //                     '<h3>'+article.titles+'</h3>'+
        //                     // '<div class="describe">'+article.desc+'</div>'+
        //                     '<span class="biao">'+article.cate_name+'</span>'+
        //                     '<span class="time">'+article.create_time+'</span>'+
        //                     '<span class="custom">评论（'+article.count+'）| 收藏（'+article.collect+'）</span>'+
        //                 '</div>'+
        //             '</li>'
        //         )
        //     }
        //  }

        function getOffer() {
            $("#getOffer").removeAttr("onclick");
            var url = "/index.php/Organization/getOfferList";
            var id = getQueryString('id');
            $.get(url, {id: id}, function (result) {
                if (result.status == 0) {
                    return alert(result.message);
                }
                var offerList = result.data.offerList;
                layer.open({
                    type: 1
                    , area: ['100%', '70%']
                    , title: '— 点击领取课程优惠券 —'
                    , shade: 0.3 //遮罩透明度
                    , anim:'up'
                    ,style: 'position:fixed; bottom:0; left:0; width: 100%; height: 70%; padding:10px 0; border:none;'
                    , shadeClose:true
                    , content: '<div>' +
                    '<ul class="offerList">' +
                    '</ul></div>'
                });
                $.each(offerList, function (key, value) {
                    var startYear=value.start_time.split(".")[0];
                    var startMonth=value.start_time.split(".")[1];
                    var startDay=value.start_time.split(".")[2];
                    var endYear=value.end_time.split(".")[0];
                    var endtMonth=value.end_time.split(".")[1];
                    var endDay=value.end_time.split(".")[2];
                    $(".offerList").append("<li id='coupon-id-"+value.id+"' onclick='getCoupon("+value.id+")'>" +
                                '<div class="content">'+
                                    '<div class="texts">'+
                                        '<span>￥<strong>'+value.subtract.split(".")[0]+'</strong></span>'+
                                        '<div>'+
                                            '<span>'+value.title+'</span>'+
                                            '<p>订单金额满'+value.full.split(".")[0]+'元使用</p>'+
                                        '</div>'+
                                    '</div>'+
                                    '<div class="hr"></div>'+
                                '<p>有效期'+startYear+'年'+startMonth+'月'+startDay+'日-'+endYear+'年'+endtMonth+'月'+endDay+'日</p>'+
                                '</div>'+
                           "</div></li>"
                           );

                    if(value.is_get == 1){
                        $("#coupon-id-"+value.id).css('background-image','url(../../../../Public/images/used.png)').attr('onclick',"goshop()");
                    }
                });
                $("#getOffer").attr("onclick","getOffer()");
                var windowWidth=$(window).width();
                if(windowWidth<680){
                    $(".offerList li").css("height","5.5rem")
                    $(".texts span strong").css("font-size","1.5rem")
                    $(".texts div p").css("font-size","0.65rem")
                    $(".texts div span").css("font-size","1rem")
                    $(".content >p").css("font-size","0.56rem")
                }
            }, 'json');
        }

        function getCoupon(id) {
            var url = "/index.php/Organization/getCoupon";
            $.get(url, {id:id}, function (result) {
                if(result.status == 0){
                    layer.closeAll();
                    layer.open({
                        content: result.message
                        ,skin: 'msg'
                        ,time: 10 //2秒后自动关闭
                    });
                }
                if(result.status == 1){
                     $("#coupon-id-"+id).css('background-image','url(../../../../Public/images/used.png)').attr('onclick',"goshop()");
                }
            }, 'json')
        }

        function getQueryString(name) {
            var reg = new RegExp("(^|&)" + name + "=([^&]*)(&|$)", "i");
            var r = window.location.search.substr(1).match(reg);
            if (r != null) return unescape(r[2]);
            return null;
        }
        //    获取url参数
        function GetRequest() {
            var url = location.search; //获取url中"?"符后的字串
            var theRequest = new Object();
            if (url.indexOf("?") != -1) {
                var str = url.substr(1);
                strs = str.split("&");
                for (var i = 0; i < strs.length; i++) {
                    theRequest[strs[i].split("=")[0]] = unescape(strs[i].split("=")[1]);
                }
            }
            return theRequest;
        }
        var id=GetRequest().id;
        function goshop(){
            window.location.href="/index.php/Organization/home.html?id="+id;
        }
    </script>
    </body>
    </html>