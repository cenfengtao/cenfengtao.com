...
class ChuanglanSmsApi {
	...
	public function sendSMS( $mobile, $msg, $needstatus = 'true') {
		...
		//创蓝接口参数
		$postArr = array (
			'account'  =>  ChuanglanConfig::API_ACCOUNT,
			'password' => ChuanglanConfig::API_PASSWORD,
			'msg' => urlencode($msg),
			'phone' => $mobile,
			'report' => $needstatus
        );
		
		$result = $this->curlPost( ChuanglanConfig::API_SEND_URL , $postArr);
		return $result;
	}
	...
	public function sendVariableSMS( $msg, $params) {
		...
		$result = $this->curlPost( ChuanglanConfig::API_VARIABLE_URL, $postArr);
		return $result;
	}
