...
class ParentingController extends BaseController
{
    ...
    public function ajaxIndex()
    {
        ...
        //亲子分类
        $class = M('parenting_cate')->order('sort desc')->select();
        //亲子团购
        if ($_GET['class_id'] == 0) {
            $groupParenting = M('group_product')->where(array('status' => [array('eq', 1), array('eq', 2), 'or'],
                'check_status' => 2, 'type' => 3, 'is_show' => 1))->limit($_GET['page'], 8)->select();
        } else {
            $groupParenting = M('group_product')->where(array('status' => [array('eq', 1), array('eq', 2), 'or'],
                'class_id' => $_GET['class_id'], 'check_status' => 2, 'type' => 3, 'is_show' => 1))->limit($_GET['page'], 8)->select();
        }
        if ($groupParenting) {
            foreach ($groupParenting as $k => $v) {
                $groupParentingCount[$k] = M('order')->where(array('group_id' => array('eq', $v['id']),
                    'status' => [array('eq', 1), array('eq', 4), 'or']))->sum('amount');
                if (!$groupParentingCount[$k]) {
                    $groupParenting[$k]['groupCount'] = 0;
                } else {
                    $groupParenting[$k]['groupCount'] = $groupParentingCount[$k];
                }
                $groupParenting[$k]['logo'] = M('organization')->where(array('token' => $v['token']))->getField('picture');
                $tag = explode(' ', $v['tag']);
                $groupParenting[$k]['tagA'] = $tag[0] ?: '';
                $groupParenting[$k]['tagB'] = $tag[1] ?: '';
                $groupParenting[$k]['tagC'] = $tag[2] ?: '';
            }
        }
        if ($_GET['page'] == 0) {
            return show(1, '', ['groupParenting' => $groupParenting, 'class' => $class]);
        } else {
            return show(1, '', ['groupParenting' => $groupParenting]);
        }
    }
