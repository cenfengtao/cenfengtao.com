...
//设置版权控件位置
    ...
    cr.addCopyright({
        id: 1,
        content: "<a href='http://blog.csdn.net/love_xiolan/article/details/52711874' style='font-size:12px;'>全屏动画背景</a>",
        bounds: bs
    });
...
//创建标注
    ...
    if (this._ismove == "1") {
        ...
        marker.addEventListener("dragend", function (e) {
            try {
                document.getElementById(poj).value = e.point.lng + "," + e.point.lat;
            } catch (ex) {
            }
        });
        //点击设置位置
...
//显示标注时
    if (this._ismove == "0") {
        ...
        if (this._pic != "") {
            content += '<img src="' + this._pic + '"style="float:right;zoom:1;overflow:hidden;width:100px;height:100px;margin-left:3px;"/>';
        }
        if (this._addr != "") {
            content += "地址：" + this._addr + "<br/>";
        }
        if (this._tel != "") {
            content += "电话：" + this._tel + "<br/>";
        }
        if (this._user != "") {
            content += "姓名：" + this._user + "<br/>";
        }
        if (this._lawfirm != "") {
            content += "简介：" + this._lawfirm;
        }
        content += "</div>";
        ...
        marker.addEventListener("mouseover", function () {
            ...
            //<span style="white-space:pre"> </span>document.getElementById('picid').onload = function (){infoWindow.redraw();}
        });
    }
}
