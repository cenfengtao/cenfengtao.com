...
$(".userBillboard").attr("href","/index.php/Vote/userBillboard.html?vote_id="+vote_id)
                ...
                if(data.is_null==0){
                    ...
                    if(data.upload_type==1){
                        ...
                        $(".works").append(
                            '<li class="to">'+
                                '<section class="img">'+
                                    ...
                                    '<div class="swiper-container" onclick="toVote('+data.id+','+data.vote_id+')">'+
                                ...
                                '</section>'+
                                '<section class="name">'+
                                    '<p><i></i>作品名称：<span>'+data.title+'</span></p>'+
                                '</section>'+
                                '<section class="username">'+
                                    '<p><i></i>作者姓名：<span>'+data.title+'</span></p>'+
                                '</section>'+
                                '<section class="nowTicket">'+
                                    '<div>'+
                                        '<p><i></i>当前票数：<span>'+data.vote_count+'</span>票</p>'+
                                        '<p style="margin-left:0.94rem;"><i></i>作品状态：<span class="status"></span></p>'+
                                    '</div>'+
                                '</section>'+
                                '<section class="date">'+
                                    '<p><i></i>投稿日期：<span>'+year+'年'+month+'月'+day+'日</span></p>'+
                                '</section>'+
                                '<section class="startEnd">'+
                                    '<p><i></i>活动起止时间：<span>'+year+'年'+month+'月'+day+'日</span></p>'+
                                '</section>'+
                                '<section class="go vote1" style="display:none;text-align:center;">'+
                                    // '<a href="javascript:void(0);" class="sendVote">转发拉票</a>'+
                                    '<a href="javascript:void(0)" class="goVote" onclick="createVotePoster()" style="background:#db6066;">生成海报拉票</a>'+
                                '</section>'+
                                '<section class="go vote2" style="display:none;">'+
                                    '<a href="javascript:void(0);" onclick="revise(event,'+data.id+','+data.vote_id+')" class="revise">我要修改</a>'+
                                    '<a href="javascript:void(0);"  class="welcome">邀请好友参加</a>'+
                                '</section>'+
                            '</li>'
                        )
                        ...
                        if(data.voteStatus==1){
                           if(data.status==1){
                                document.getElementsByClassName("status")[0].innerHTML="已结束";
                                 $(".startEnd").find("p").css("border-bottom","none")
                            }else if(data.status==2){
                                document.getElementsByClassName("status")[0].innerHTML="已结束";
                                $(".vote1").show().html('<a href="javascript:void(0)" onclick="alerts(event)" class="goVote" style="background:gray;">去拉票</a>');
                        ...
                        if(data.voteStatus==2){
                            if(data.status==1){
                                $(".vote2").show()
                                document.getElementsByClassName("status")[0].innerHTML="审核中";
                            ...
                            if(data.status==2){
                                $(".vote1").show()
                                document.getElementsByClassName("status")[0].innerHTML="投票中";
                            ...
                            if(data.status==3){
                                $(".vote2").show()
                                document.getElementsByClassName("status")[0].innerHTML="落选了";
                                document.getElementsByClassName("goVote")[0].style.display="none";
                                document.getElementsByClassName("revise")[0].style.marginTop="0";
                                document.getElementsByClassName("to")[0].onclick=function(){};
                            }
                    ...
                    if(data.upload_type==2){
                        ...
                        $(".works").append(
                            '<li class="to">'+
                                '<section class="img" onclick="toVote('+data.id+','+data.vote_id+')">'+
                                    '<video src="'+data.video+'" preload controls poster="'+data.path+'" controlsList="nodownload"></video>'+
                                '</section>'+
                                '<section class="name">'+
                                    '<p><i></i>作品名称：<span>'+data.title+'</span></p>'+
                                '</section>'+
    ...
    function alerts(event){
        stopBubble()
        alert("该作品的投票活动已结束，点击图片/视频可查看该作品之前的作品详情。")
    }
